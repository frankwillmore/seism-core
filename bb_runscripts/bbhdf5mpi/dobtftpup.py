...
def main(argv):
    ...
    binary_fname = ''
    qatestdir = ''
    for btparam in json_data[buildconfig][ctparams]:
        if 'testparams' in btparam:
            testparams = json_data[buildconfig][ctparams]['testparams']
            #print ('tconf test parms = {}'.format(testparams))

            for testparam in testparams:
                #print ('tconf tparm = {}'.format(testparam))
                if 'publish' in testparam:
                    publish = testparam['publish']
                if 'publishsource' in testparam:
                    publishSource = testparam['publishsource']
                            ...
                            for binfile in envparams['binary']:
                                for k,v in binfile.items():
                                    ...
                                    if slaveplatform.lower() in ['vs11', 'vs12', 'vs14']:
                                        ...
                                        if 'btctest' == buildsys or 'ctest' == buildsys:
                                            binary_fname = cmake_to_binary(v, slaveplatform.lower(), slaveos.lower())
                                        else:
                                            binary_fname = linux_to_binary(v, slaveplatform.lower(), slaveos.lower())
                        print ('publish binary is {}'.format(binary_fname))
                elif publishSource:
                            ...
                            for k,v in binfile.items():
                                ...
                                if slaveplatform.lower() in ['vs11', 'vs12', 'vs14']:
                                    ...
                                    if 'btctest' == buildsys or 'ctest' == buildsys:
                                        ...
                                        binary_fname = linux_to_binary(v, slaveplatform.lower(), slaveos.lower())
                    print ('publishSource binary is {}'.format(binary_fname))
    ...
    print ('publish {} to {}'.format(binary_fname,qatestpath))
    ...
    if sftpclient is not None:
        ...
        currdir=os.getcwd() ####
        print ('current2={}'.format(currdir))

        try:
            filename = '%s' % binary_file
            print ('Putting {}'.format(filename))
