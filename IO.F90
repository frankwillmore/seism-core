...
!! subroutines in case multiple precisions are used in the code.
!!
!! NOTES:
!!
...
MODULE IO
   ...
   INTERFACE READ_DATA
      MODULE PROCEDURE READ_DATA_
      MODULE PROCEDURE READ_DATA_PARTIAL
...
CONTAINS
   ...
   !> @param[in] h5prec HDF5 identifier for the variable type.
   SUBROUTINE READ_ATTRIBUTE(hdf5_id, att_name, var, h5prec)
      IMPLICIT NONE
      ! Calling arguments.
      INTEGER(KIND=HID_T),INTENT(IN) :: hdf5_id
      CHARACTER(LEN=*),INTENT(IN) :: att_name
      TYPE(C_PTR), VALUE :: var
      INTEGER(KIND=HID_T), INTENT(IN) :: h5prec
      ! Local variables.
      ! HDF5 identifier for the attribute.
      ...
      !
      ! Read the attribute.
      CALL H5AREAD_F(att_id, h5prec, var, ierr)
      !
      ! Close the attribute.
   ...
   SUBROUTINE WRITE_SLICE(dims, dname, dat, h5prec, hdf5_id, srank, sind)
      ! Required modules.
      USE ISO_FORTRAN_ENV,ONLY: REAL32
      IMPLICIT NONE
      ! Calling arguments.
      ...
      CALL H5DWRITE_F(dset_id, h5prec, dat, ierr, &
   ...
   END SUBROUTINE WRITE_SLICE_PARALLEL
   ...
   SUBROUTINE READ_DATA_(hdf5_id, buf, h5prec, dims, dname, offset)
      IMPLICIT NONE
      ! Calling arguments.
      INTEGER(KIND=HID_T),INTENT(IN) :: hdf5_id
      ...
      !
      ! Identify the HDF5 type.
      !h5prec = H5KIND_TO_TYPE(buf_kind, H5_INTEGER_KIND)
      !
      ! Open the dataset.
      ...
      CALL H5DREAD_F(dset_id, h5prec, buf, ierr, memspace, dspace_id)
      !
   ...
   SUBROUTINE READ_DATA_PARTIAL(hdf5_id, dims, buf, h5prec, dname, &
      ...
      INTEGER(KIND=HSIZE_T),DIMENSION(:),INTENT(IN) :: dims
      TYPE(C_PTR), VALUE :: buf
      INTEGER(HID_T), INTENT(IN) :: h5prec
      CHARACTER(LEN=*),INTENT(IN) :: dname
      INTEGER(KIND=HSIZE_T),DIMENSION(*),INTENT(IN) :: dims_slab
      INTEGER(KIND=HSIZE_T),DIMENSION(*),INTENT(IN) :: offset_read, offset_fill
      ! Local variables.
      ...
      ! Close the dataset.
      CALL H5DCLOSE_F(dset_id, ierr)
   END SUBROUTINE READ_DATA_PARTIAL
